...
const NoData:React.FC<NoDataProps>=({message, imageUrl, description, onclick, buttonText="Try Again"}) => {
  return (
    ...
    {onclick && (
      <button
      onClick={onclick}
      className='px-6 w-60 py-3 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-300 text-white font-medium rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transform transition duration-300 ease-in-out cursor-pointer'
      >
        {buttonText}
      </button>
    )}
