...
const AuthPage: React.FC<LoginProps> = ({isLoginOpen, setIsLoginOpen}) => {
	...
	const {register:registerForgotPassword, handleSubmit: handleForgotPasswordSumbmit, formState: {errors:forgotPasswordError}} = useForm<ForgotPasswordFormData>();
		...
		<DialogContent className='sm:max-w-[425px] p-6'>
			<DialogHeader>
				...
				<AnimatePresence mode='wait'>
					...
					<TabsContent value='login' className='space-y-4'>
						...
						<div className='flex items-center my-4'>
							<div className='flex h-px bg-gray-300'></div>
							<p className='mx-2 text-gray-500 text-sm'>Or</p>
							<div className='flex h-px bg-gray-300'></div>
						</div>
						<Button
						className='w-full flex items-center gap-2 bg-white text-gray-700 border border-gray-300 hover:border-gray-50'
						>
						{googleLoading ? (
						<>
						<Loader2 className='animate-spin mr-2' size={20} /> Login with Google...
						</>
						) : (
						<>
						<Image  alt='login' src={'/icons/google.svg'} width={20} height={20} />
						Login with Google
						</>
						)}
						</Button>
					...
					<TabsContent value='signup'>
						...
						action="">
							...
							<div className='flex items-center'>
								<Input
								type='checkbox'
								{...registerSignUp('agreeTerms', {required:'you must agree to the terms and conditions'})}
								className='mr-2'
								 />
					...
					<h3 className='text-xl font-semibold text-gray-700'>Reset Link Send</h3>
				...
				<p className='text-sm mt-2 text-center text-gray-600'>
					By clicking `agree`, you are to our {' '}
